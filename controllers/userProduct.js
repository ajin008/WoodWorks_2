...
exports.orderPlacement_1 = async (req, res, next) => {
  try {
    ...
    if (oderType === "wallet") {
      ...
      if (wallet) {
        ...
        wallet.transactions.push({
          type: "withdrawal",
          amount: orderTotal,
          description: `Order payment for ${productName}`,
          status: "success",
          orderId: couponId,
        });
        await wallet.save();
    ...
    const remainingQuantity = product.qty - quantity;
    ...
    console.log("remaining qty:", product.qty);
...
exports.createRazorpayOrder = async (req, res) => {
  try {
    ...
    // Ensure amount is being received correctly
    const amount = req.body.orderTotal;
    if (!amount) {
    ...
    if (error instanceof Razorpay.errors.BadRequestError) {
      return res.status(400).json({ error: "Bad request" });
    } else if (error instanceof Razorpay.errors.AuthenticationError) {
      return res.status(401).json({ error: "Authentication error" });
    } else if (error instanceof Razorpay.errors.ForbiddenError) {
      return res.status(403).json({ error: "Forbidden error" });
    } else if (error instanceof Razorpay.errors.NotFoundError) {
      return res.status(404).json({ error: "Not found error" });
    } else if (error instanceof Razorpay.errors.ServerError) {
...
exports.verifyRazorpayPayment = async (req, res) => {
  try {
    ...
    const updateResult = await MyOrder.updateOne(
      { orderId: orderId },
      { onlinePaymentStatus: paymentResponse ? "success" : "failed" }
    );

    if (updateResult.modifiedCount === 0) {
      console.error("Order not found or status not updated");
      return res
        .status(404)
        .json({ error: "Order not found or status not updated" });
    }

    console.log(
      `Payment verified successfully. onlinePaymentStatus set to ${
        paymentResponse ? "success" : "failed"
      }.`
    );
...
exports.RenderingOder_detail = async (req, res, next) => {
  try {
    ...
    const activeOrders = await MyOrder.find({
      userId: userId,
      status: { $ne: "Delivered" },
    });

    // Fetch canceled orders for the given user from the CanceledOrder collection
    const canceledOrders = await CanceledOrder.find({
      userId: userId,
    });

    // Define a function to fetch additional product details for each order
    const populateOrderDetails = async (order, isCancelled = false) => {
      ...
      return {
        ...
        offerPrice: product.offerPrice,
        orderId: order._id,
        status: isCancelled ? "Order Cancelled" : order.status,
    ...
    // Map over the active and canceled orders arrays and populate additional product details for each order
    const populatedActiveOrders = await Promise.all(
      activeOrders.map((order) => populateOrderDetails(order))
    );
    const populatedCanceledOrders = await Promise.all(
      canceledOrders.map((order) => populateOrderDetails(order, true))
    );

    // Combine active and canceled orders
    const allOrders = [...populatedActiveOrders, ...populatedCanceledOrders];

    // Sort orders by creation date
    allOrders.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    // Render the OderDetails view and pass the orders data to it
    res.render("OderDetails", { orders: allOrders, userId: userId });
  } catch (error) {
...
exports.RenderingDeliveredOder_detail = async (req, res) => {
  ...
  try {
    ...
    // Iterate over each delivered order
    for (const order of deliveredOrders) {
      // Check if order has productId and returnStatus is not "Accepted"
      if (order.productId && order.returnStatus !== "Accepted") {
        // Format deliveryDateTime
        const formattedDeliveryDateTime = order.deliveryDateTime.toLocaleString(
          "en-US",
          {
...
exports.viewTransaction = async (req, res, next) => {
  try {
    ...
    // Filter transactions to include only successful ones
    const successfulTransactions = wallet.transactions.filter(
      (transaction) => transaction.status === "success"
    );

    console.log("data in the wallet", successfulTransactions);

    // Send the filtered transactions in the response
    res.json({ transactions: successfulTransactions });
  } catch (error) {
    next(error);
  }
};
